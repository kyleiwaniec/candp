...
         }); // end .get function
	...
	var graphPath = r.path().attr({stroke: '#ff0066', 'stroke-width': 5});
    ...
    for (i=1;i<=n;i++) {
          ...
          if(i == 1){
                            textToInsert[j++] = '<tr><td colspan="7" style="background-color:#ff0066; color:white">Year '+1+'</td></tr>';
                        }else if(i%12 == 0){
                            textToInsert[j++] = '<tr><td colspan="7">Year '+(i/12 + 1)+'</td></tr>';
                        }
	...
	$("#amorttable").append("<table cellspacing='0' cellpadding='0' class='table'><tr><th>Month</th><th>Payment</th><th>Principal</th><th>Interest</th><th>Total<br />Payments</th><th>Total<br />Interest</th><th>Balance<br />Remaining</th></tr>" + textToInsert.join('') + "</table>").css({"display": "none"});
        $("table tr td:first-child").css({"width":"25px"});
	...
	$("table th").css({"background-color":"#ffffff", "vertical-align" : "bottom", "color" : "#333333"});
        $("table tr:nth-child(13n+1)").css({"background-color":"#ff0066",  "color":"white"});
};

...
<script type="text/javascript">
	...
	// the images from flickr //
        var path = "http://api.flickr.com/services/feeds/photos_public.gne?tags=ocean,blue,green,water&tagmode=all&format=json&jsoncallback=?";
        var flkImgs = [];
...
//                //$("<img/>").attr("src", data.items[i].media.m).appendTo("body");
//                //flkImgs.push(data.items[i].media.m);
//                flkImgs[i] = data.items[i].media.m;
//                return flkImgs;
//                }
//            console.log(flkImgs[1]);
////                $.each(data.items, function(i,item){
////                    flkImgs[i] = item.media.m;
...
//            });
        ...
        $(window).resize(function(){
            ...
            $('.panel, .front').css({width  : panelWid, 
                                     height : panelHei});
...
//                    ];
        ...
        $.getJSON(path, function(data){
                        ...
                        for(var i = 0; i < data.items.length; i++){
                            //$("<img/>").attr("src", data.items[i].media.m).appendTo("body");
                            //flkImgs.push(data.items[i].media.m);
                            flkImgs[i] = "url("+data.items[i].media.m+")";
        ...
        for(var i = 0; i < flkImgs.length; i++){
            var card = $('<div>', {
                'class': 'click panel',
                css: {
                    width: panelWid, 
